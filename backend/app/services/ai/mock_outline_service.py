...
def generate_mock_knowledge_outline(lesson: Lesson, materials: list[LessonMaterial]) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            "## 2. 学习目标",
            "- 基础必达目标：能说出本课核心概念，并完成基础练习。",
            "- 提高目标：能根据任务要求修改示例并排查常见错误。",
            "- 拓展目标：能说明本课知识在相邻课次或简单职业场景中的应用方向。",
            "",
            "## 3. 核心知识点",
            ...
            "",
            "## 5. 重点与难点",
            "- 教学重点：围绕本课核心概念、关键语法或操作步骤展开。",
            "- 教学难点：学生容易在适用条件、步骤顺序或结果核验上出现偏差。",
            "",
            "## 6. 材料结构分析与教学重心提醒",
            "| 分析维度 | 提醒内容 | 建议处理 |",
            "|---|---|---|",
            "| 已覆盖内容 | 当前材料可支持本课基础知识梳理 | 可保留并结合课堂任务强化 |",
            "| 可能缺口 | 可能缺少分层练习、易错提醒或结果核验示例 | 建议补充 |",
            "| 不宜放大内容 | 6S、课堂纪律等如非本课主题，宜作为管理提醒 | 降级为提醒 |",
            "| 更适合的教学中心 | 聚焦核心知识、任务完成质量和结果核验 | 建议强化 |",
            "| 需教师确认 | 课程标准、真实案例、学校制度等 | 需教师确认 |",
            "",
            "## 7. 课程思政与职业素养融入点",
            ...
            "",
            "学生导学案素材提取：可将本课关键词转化为学生填空题，将基础操作步骤转化为过程记录，将查询结果核验、代码规范或安全规范转化为职业素养提醒，并从核心概念、前置知识、易错点和操作规范中选择内容设计课前学情测试。",
            "",
            "## 11. 补充内容建议",
